# Family Context

Extra context to help the AI interpret calendar events accurately.

## People
- **Dennis** — Dad. Risk Manager at Instacart (day job). Runs Glacier Point Insurance (side business).
- **Amy** — Mom. Works at Orinda USD.
- **Anna** — 12 year old daughter. 7th grade. Attends Stanley Middle School.
- **Sophia** — 10 year old daughter. 5th grade. Attends Lafayette Elementary School.

## Sports & Activities
- "Springbrook" or "swim" → Sophia's swim team (Springbrook Swim Team). Practice times: 10&up group is 5:35 PM - 6:20 PM. Bring competitive suit, goggles, swim cap. Team contact: Lauren Deal.
- "Luna" or "gymnastics" → Sophia's gymnastics at Luna Gymnastics in Moraga. Super Stars class, Wednesdays at 2:30 PM with Coach Wayne.
- "volleyball" → Both Anna and Sophia play volleyball. Registered through LMYA Sports.
- "LAMO" → Anna's soccer team (LAMO Soccer / Lamorinda Soccer Club). Private lessons with Coach Luis.
- "Eclipse" → Sophia's soccer team (East Bay Eclipse Soccer Club). Spring program starts March 7.
- "Tech rehearsal" or "Alice in Wonderland" → School play/performance rehearsal. Recent: Town Hall Theatre Company's Alice in Wonderland production.

## Medical / Dental
- **Family Dentist** — Milcovich Dental Arts, Dr. William Milcovich, 1855 San Miguel Dr., Suite 27, Walnut Creek, CA 94596. Phone: (925) 944-5151. Website: milcodental.com
- **Sophia's Orthodontist** — Sophia has braces/orthodontist appointments (approximately 1 hour long). Scheduled during school hours; needs afternoon pickup at 1:20 PM from school.

## Schools
- **Lafayette Elementary School** — Sophia's school. Lafayette, CA. Contact: Melissa Barclay (Principal).
- **Stanley Middle School** — Anna's school. Contact: Brian Mangold.

## Locations
- Lafayette Elementary School — Lafayette, CA
- Stanley Middle School — Orinda, CA
- Luna Gymnastics — 594 Moraga Rd, Moraga, CA
- Milcovich Dental Arts — 1855 San Miguel Dr., Suite 27, Walnut Creek, CA
- Springbrook Pool — Lafayette
- Town Hall Theatre Company — Local school theater productions

## Recurring Bills & Deadlines (Flag These as Action Items)

### Property Taxes
- **Contra Costa County** — 1st installment due Dec 10, 2nd due April 10. Late = 10% penalty.
- **Placer County** — Property in Placer County (parcel 990521204000 and 103150023000). Autopay enrolled via Invoice Cloud. Still flag when payment confirmations arrive.

### Taxes
- **Estimated taxes** — Federal/CA quarterly estimates. Due dates: April 15, June 15, Sept 15, Jan 15.
- **NJ Attorney Registration** — Dennis holds an NJ law license. Annual registration + payment required (reminder emails from NJCourtNotice). Flag these — penalty for non-compliance.

### Mortgage & Housing
- **Chase mortgage** — Auto-pay enrolled (account ending 3425). Payment scheduled monthly. Flag the "scheduled" notice so Dennis can verify amount.

### Insurance (Personal)
- **PURE Insurance** — Homeowners/umbrella. Auto-pay via checking. Recent payment: $2,283.84 (March 2026). Flag renewal notices.
- Flag any homeowners, auto, umbrella renewal notices. Dennis is an insurance professional — he should never let personal policies lapse.

### Utilities (All on Auto-Pay)
- **PG&E** — Account ending 5740-2. Autopay enrolled. Recent bill: $523.40. Flag if amount spikes or if autopay fails.
- **Xfinity** — Account ending 9644. Auto-pay $70/mo.
- **Verizon Wireless** — Auto-pay enrolled. Flag bill-ready notices.

### Credit Cards
- **Chase United Quest Visa** — Ending 2529. Recent statement: $3,532.78, due 04/14/2026.
- **Costco Anywhere Visa (Citi)** — Ending 7676. Cardmember since 2019. Statements arrive mid-month.

### Banking
- **Chase** — Primary checking (ending 0596), additional account (ending 3425 for mortgage).
- **U.S. Bank** — Checking ending 4582. Monthly e-statements.
- **Fidelity** — Instacart 401(k)/retirement plan. Flag any "Action Needed" emails from Fidelity — these are time-sensitive (e.g., rollovers, tax notices).

### Subscriptions
- **Cloudflare** — Monthly invoice (website hosting for glacierpointinsurance.com).
- **Apple** — Poker Trainer yearly subscription (renews March 21).
- **Google Ads** — Active campaign for Glacier Point. Flag billing issues only.

### Kids Activities & Registrations
- **LMYA Sports** — Anna's volleyball team is "Myrtle" (6th grade, practice Thu 7pm). Sophia's team is "Manhattan" (4th grade, practice Wed 6pm, games Sat 2:30pm). Flag registration deadlines.
- **Springbrook Swim Team** — Summer team registration opens ~February. "HOGS" = the Springbrook team nickname. Early bird pricing has deadlines. Also: annual clean-up day, pool key pickup.
- **East Bay Eclipse Soccer** — 2026-27 tryout registration is open. Flag tryout dates and deadlines.
- **Town Hall Theatre Company** — Alice in Wonderland production. Volunteer sign-ups required.
- **School fees/forms** — Field trips, yearbook orders, picture day, etc. that require payment or a signed form by a deadline.

## Itinerary Preferences
- **Action items**: Prioritize upcoming bills, payments, auto-withdrawals, and due dates — anything with a penalty for being late (taxes, insurance premiums, utilities, subscriptions, tuition). Include amount and due date when available.
- **Upcoming travel**: Only include trips involving flights or leaving the Bay Area (e.g., Disneyland, ski trips). Exclude local Bay Area activities.
- **Skip**: PTA meeting invites — Dennis does not attend PTA meetings.
- **Skip**: Glacier Point / business emails — business is handled separately. This itinerary is personal/family only.
- **Flag senders**: Emails from IRS, FTB, Contra Costa County, Placer County, NJCourtNotice (NJ bar), Chase mortgage, PURE Insurance, Fidelity "Action Needed", or any tax/CPA firm should always be flagged as action items.
- **Skip senders**: Google Ads promo emails, Google Cloud product updates, CVS promotions, Costco Visa FICO score updates, Chase "see how your home compares" marketing.
- **Done label**: Dennis labels emails "Done" in Gmail when he's handled them. The scanner excludes anything labeled Done. This is how items drop off the daily email.
- **Tiered action items**: Items are sorted into DUE NOW (red, ≤3 days), THIS WEEK (amber, ≤7 days), and ON RADAR (gray, 7-14 days). Most mornings should have 1-2 items, not 5.
- **Auto-skip**: Payment confirmations ("thanks for your payment") are not action items — the bill is already paid.
- **AI refinement**: The AI should continue learning from patterns in Dennis's email and calendar to improve what it surfaces vs. filters out over time. When a new recurring bill or obligation is detected, note it here for future runs.

## Notes
- Swim practice happens rain or shine unless heavy storm/lightning.
- Dennis uses Oura Ring for health tracking (stress, sleep, blood pressure monitoring).
- Family uses Warby Parker for eyeglasses/contacts.
- School communications come via **ParentSquare** (Aeries Communications) to Dennis's personal email — both Lafayette Elementary and Stanley Middle School send daily digests. Flag attendance notices (tardy/absent) and anything requiring a form or permission slip.
- **Stanley Middle School** also sends: progress reports, conference scheduling, minimum days, survey permission forms (e.g., CA Healthy Kids Survey).
- **Lafayette School District** sends: board meeting notices, budget/parcel tax info. These are FYI — don't flag as action items unless a vote or deadline is involved.
- **EBMUD** — local water utility. Sends construction/paving notices for the neighborhood. FYI only, not an action item.
